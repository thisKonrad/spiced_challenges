...
console.log("001: ",firstAnimalStartingWithLetterG);


const indexOfAnimalWithNameLongerFive = animals.filter((animal)=> {
  if( animal.name.length > 5){
    return animal;
  }
});
console.log("002: ",indexOfAnimalWithNameLongerFive);
...
const animalsSortedByWeightStartingWithLowest = 
animals.map((animal)=> animal.weight).sort((a,b)=> a - b );
...
const allAnimalsInEuropeWeighLessThanOnehundred = animals.filter(
  (animals)=> animals.continents.includes('Europe')).every(animal=> animal.weight < 100);
console.log("allAnimalsInEuropeWeighLessThanOnehundred: ",allAnimalsInEuropeWeighLessThanOnehundred)


// Hint: filter + map + reduce
const weightOfAllAnimalsInAfrica = 
animals.filter(animal=> animal.continents.includes('Africa')).map((animal)=>animal.weight).reduce((a,b)=> a+b);
